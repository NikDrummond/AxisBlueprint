...
class LayoutDesigner:
    ...
    def load_layout(self):
        ...
        templates_dir = os.path.join(os.getcwd(), "templates")
        filename = filedialog.askopenfilename(initialdir=templates_dir,
...
class SaveTemplateDialog(tk.Toplevel):
    def __init__(self, master, json_str):
        ...
        self.json_str = json_str
        tk.Label(self, text="Enter a name for your layout (without .json):").pack(pady=5)
...
def FigureFromLayout(layout_name):
    ...
    if not os.path.isfile(filepath):
        raise FileNotFoundError(f"Template file {layout_name}.json not found in templates folder.")
