...
class AxisBox:
    ...
    def is_outside_margins(self, margin):
        return (
            self.x < margin or
            self.y < margin or
            self.x + self.width > A4_WIDTH_CM - margin or
            self.y + self.height > A4_HEIGHT_CM - margin
        )
...
class LayoutDesigner:
    def __init__(self, master):
        ...
        self.btn_optimize.pack(side=tk.LEFT)
        self.btn_preview = tk.Button(self.toolbar, text="Preview JSON", command=self.preview_json)
        self.btn_preview.pack(side=tk.LEFT)
        self.btn_load = tk.Button(self.toolbar, text="Load Layout", command=self.load_layout)
        self.btn_load.pack(side=tk.LEFT)
        ...
        self.entry_margin.bind("<Return>", lambda e: self._update_margin())

        tk.Label(self.mode_frame, text="Grid Spacing (cm)").pack(pady=(15, 0))
        self.entry_grid = tk.Entry(self.mode_frame, width=10)
    ...
    def _update_margin(self):
        try:
            self.margin_cm = float(self.entry_margin.get())
            self.redraw()
        except ValueError:
            pass
    ...
    def _update_grid(self):
        try:
            self.grid_spacing_cm = float(self.entry_grid.get())
            self.redraw()
        except ValueError:
            pass
    ...
    def redraw(self):
        ...
        for i, box in enumerate(self.boxes):
            ...
            x2 = (box.x + box.width) * SCALE
            y2 = (box.y + box.height) * SCALE
            if box.is_outside_margins(self.margin_cm):
                self.canvas.create_rectangle(x1, y1, x2, y2, outline="black", fill="red", width=2)
            else:
                color = "red" if box == self.selected_box else "blue"
                self.canvas.create_rectangle(x1, y1, x2, y2, outline=color, width=2)
            label_x = (x1 + x2) / 2
            label_y = (y1 + y2) / 2
            self.canvas.create_text(label_x, label_y, text=str(i + 1), fill="black", font=("Helvetica", 12, "bold"))
